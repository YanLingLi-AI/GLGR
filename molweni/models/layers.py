    ...
    def forward(self, G, h):
        ...
        for triple in self.all_edges:
            ...
            if srctype == dsttype == 'utterance':
                utterance_to_utterance_matrix = attn_score  # batch, heads, utters, utters
            if srctype == 'question' and dsttype == 'utterance':
                utterance_to_question_matrix = attn_score # batch, heads, utters, 1
            if dsttype == 'question' and srctype == 'utterance':
                question_to_utterance_matrix = attn_score # batch, heads, 1, utters
...
class AttentionPooling1(nn.Module):
    ...
    def forward(
        ...
        w = self.attention(hidden_states) # batch, len, 1
        attention_mask = (1.0 - attention_mask.to(dtype=next(self.parameters()).dtype)) * -10000.0
        w = w + attention_mask.unsqueeze(2)
        weights = torch.nn.functional.softmax(w, dim=1)
        summed_states = torch.sum(hidden_states * weights, dim=1)
