...
ques_type_dict = {
    'what':0, 'when':1, 'where':2, 'how':3, 'why':4, 'who':5, 'other':6
}
...
class MolweniExample(object):
    def __init__(self, qid, ques_type, content_list, dialogue_graph, ques_to_dialogue_subgraph, doc_tokens,  ques_tokens, start_position, end_position, 
                 key_inner_nodes=None, answer_text = None, answers = None, is_impossible=False, answer_utterance_id=-1):
...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        data_info['no_answer'] = torch.tensor(self.features[index].is_impossible, dtype=torch.float)
        ...
        data_info['graph_nodes_dict'] = {
            ...
            'utterance_mask':torch.tensor(graph_info_dict['utterance_mask'], dtype=torch.long),
            'ques_type':torch.tensor(graph_info_dict['ques_type'], dtype=torch.long),
            'question_involve_inner_nodes':graph_info_dict['question_involve_inner_nodes'],
        }
...
class MolweniProcessor(DataProcessor):
    ...
    def _create_examples(self, data, is_training):
        ...
        find = False 
        for ui, utterance in enumerate(data["edus"]):
            ...
            speaker = utterance['speaker'].strip().lower()
        ...
        for uidx, utterance_text in enumerate(content_list):
            ...
            context_text += utterance_text.lower() + self.sep_token + ' '
        ...
        for uidx, utterance_text in enumerate(content_list):
            ...
            for i, pos_tag_info in enumerate(utter_pos_tag_info['pos']):
                ...
                if pos_tag_info[1] in ("NOUN", "PROPN"):
                    ...
                    # print(doc_tokens[word_position[0]: word_position[1]+1])
            ...
            for srl_dict in srl_data[str(uidx)]["semantic_role_labeling"]:
                ...
                assert utterance_text[sub_root_char_pos[0]:sub_root_char_pos[1]] == sub_root_info[1]
                ...
                if args.ARG_ARGM:
                    if len(arg_nodes) == 1:  # 如果只有个核心语义节点，则所有附加语义节点与核心语义节点连接
                ...
                for nid2 in range(nid1+1, len(all_inner_nodes_recover_name)):
                    ...
                    for posi in doc_tokens_posnoun:
                        if ns2<=posi[0] and ne2>=posi[1]:
                            is_noun2 = True
                            break
                    if is_noun2 and node_name2.lower() not in stop_words and len(node_name2) > 2:
        ...
        for qa in data["qas"]:
            ...
            ques_type = 6
            for qword, qidd in ques_type_dict.items():
                if qword in qas_tokens:
                    ques_type = qidd
                    break
            ...
            if is_training:
                if qa['is_impossible'] or len(qa['answers']) == 0:  # 如果不可以回答的话
                    exp = MolweniExample(qid, ques_type, content_list, dialogue_graph, ques_to_dialogue_subgraph, doc_tokens, qas_tokens, -1, -1, is_impossible=True)
                    examples.append(exp)
                    continue   # 跳过，是无法回答的问题
                for a_id, answer in enumerate(answers):
                    key_inner_nodes = []
                    answer_text = answer["text"].lower()
                    inner_start = context_text.find(answer_text)
                    start_position = char_to_word_offset[inner_start]
                    end_position = char_to_word_offset[inner_start + len(answer_text)-1]
                    answer_utterance_id = -1
                    if end_position <= utterances_index[0]:
                        answer_utterance_id = 0
                    else:
                        for i in range(1, len(utterances_index)):
                            if start_position > utterances_index[i-1] and end_position <= utterances_index[i]:
                                answer_utterance_id = i
                                break
                    # If the answer cannot be found in the text, then skip this example.
                    actual_text = " ".join(doc_tokens[start_position : (end_position + 1)]).lower()
                    cleaned_answer_text = " ".join(whitespace_tokenize(answer_text)).lower()
                    if actual_text.find(cleaned_answer_text) == -1:
                        # logger.warning("Could not find answer: '%s' vs. '%s' %s", answer_text, inner_start, context_text[inner_start:inner_start + len(answer_text)])
                    ...
                    for node, posi in all_finegaine_nodes.items():
                        if posi == [start_position, end_position]:
                            ...
                            continue
                        elif posi[0] <= start_position and posi[1] >= end_position:
                            key_inner_nodes.append(node)
                            continue
                    ...
                    exp = MolweniExample(qid, ques_type, content_list, dialogue_graph, ques_to_dialogue_subgraph, doc_tokens, qas_tokens, start_position, end_position, 
                                         key_inner_nodes, answer_text, answer_utterance_id = answer_utterance_id)
                    examples.append(exp)
            else:
                if qa['is_impossible'] or len(qa['answers']) == 0:  ans = []
                else:
                    ans = [a["text"] for a in answers]
                examples.append(MolweniExample(qid, ques_type, content_list, dialogue_graph, ques_to_dialogue_subgraph, doc_tokens, qas_tokens, None, None, answers = ans))
...
def molweni_convert_example_to_features(
    ...
    for span in spans:
        ...
        start_position = 0; end_position = 0; span_is_impossible = example.is_impossible
        ...
        # print('remove_finegain_nodes',remove_finegain_nodes)
        ...
        # srl-srl
        # srl_mask = np.zeros((len(span["input_ids"]), len(span["input_ids"])))
        # srl_type = np.zeros((len(span["input_ids"])))
        ...
        question_involve_inner_nodes = [0]*MAX_UTTERANCE_INNER_NODE_NUM
        for edge in q_noun_to_final_dialogue_edges:
            ...
            if e < MAX_UTTERANCE_INNER_NODE_NUM:
                ...
                question_involve_inner_nodes[e] = 1
        ...
        if not utterance_to_speaker_s:
            print(span['tokens'])
            print(utter2spk)
            print(utter_nodes_to_ids)
            print(finegain_nodes_to_ids)
            print('=====================================')

        query_mapping = np.zeros_like(span["attention_mask"])
        query_mapping[:query_end] = 1
        
        utterance_mask = np.zeros_like(span["attention_mask"])
        for nodeid, idxs in new_utter_nodes_to_token_position.items():
            if nodeid >= MAX_UTTERANCE_NUM:
                continue
            for idx in range(idxs[0], idxs[-1]+1):
                utterance_mask[idx] = 1
        for i in range(len(span["input_ids"])):
            if span["input_ids"][i] == tokenizer.pad_token_id:
                utterance_mask[i : i + 1] = 0
        ...
        # print(new_utter_nodes_to_token_position)
        graph_info_dict = {
            ...
            # 'srl_mask':srl_mask, 'srl_type':srl_type, 'utter_self_mask':utter_self_mask, 'coref_mask':coref_mask,
            'answer_utterance_id':answer_utterance_id, 'utter_p_mask':utter_p_mask,
            'key_inner_nodes':key_inner_nodes, 'utterance_mask':utterance_mask,
            'tokens':encoded_dict["tokens"],
            'ques_type':[example.ques_type], 'question_involve_inner_nodes':question_involve_inner_nodes
        }
