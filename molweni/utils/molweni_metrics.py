...
def get_raw_scores(examples, preds):
    ...
    exps = []; exs = []
    all_false_num = 0; file_num = 0; biaozhu = 0; all_same_ques = 0
    all_first_utr = {}; all_ques = []; num = 0
    for example in examples:
        qas_id = example.qas_id
        gold_answers = [answer for answer in example.answers if normalize_answer(answer)]

        if not gold_answers:   # For unanswerable questions, only correct answer is empty string
            gold_answers = [""]
        ...
        #     print()
        #     print(qas_id)
        #     for uidx, utterance_dict in enumerate(example.utterances):
        #         text = utterance_dict['text'].replace("\n"," ").replace("\t"," ")
        #         speaker = utterance_dict['speaker'].replace("\n"," ").replace("\t"," ")
        #         print(speaker+': \t'+text)
        #     print('qiestion:', example.question)
        #     print('predict:', prediction)
        #     print('true:', gold_answers)
...
def compute_predictions_logits(
    ...
    if output_null_log_prob_file:
        logger.info(f"Writing null_log_prob to: {output_null_log_prob_file}")
    ...
    all_no_answers = collections.OrderedDict()
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        all_no_answers[example.qas_id] = example_no_answer_score
    ...
    if write_predictions:
        ...
        if output_null_log_prob_file:
            with open(output_null_log_prob_file, "w") as writer:
                writer.write(json.dumps(all_no_answers, indent=4) + "\n")
                
    return all_predictions, all_no_answers
