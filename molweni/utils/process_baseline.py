...
class MolweniExample(object):
    def __init__(self, qid, content_list, doc_tokens,  ques_tokens, start_position, end_position, answer_text = None, answers = None, is_impossible=False):
...
class MolweniProcessor(DataProcessor):
    ...
    def _create_examples(self, data, is_training):
        ...
        for ui, utterance in enumerate(data["edus"]):
            content_list.append(srl_data[str(ui)]['utterance'].lower())
        ...
        for qa in data["qas"]:
            ...
            # if question.lower() != "how does qkslvrwolf use the different icon ?":
            #     continue
            qas_tokens = []
            ...
            if is_training:
                if qa['is_impossible'] or len(qa['answers']) == 0:  # 如果不可以回答的话
                    exp = MolweniExample(qid, content_list, doc_tokens, qas_tokens, -1, -1, is_impossible=True)
                    examples.append(exp)
                ...
                for a_id, answer in enumerate(answers):
                    ...
                    exp = MolweniExample(qid, content_list, doc_tokens, qas_tokens, start_position, end_position, answer_text)
                    examples.append(exp)
            else:
                if qa['is_impossible'] or len(qa['answers']) == 0:  ans = []
                else:
                    ans = [a["text"] for a in answers]
                examples.append(MolweniExample(qid, content_list, doc_tokens, qas_tokens, None, None, answers = ans))
        return examples
